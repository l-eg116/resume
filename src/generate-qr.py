...
def add_corners(im: Image, rad: int) -> Image:
    """Add transparent rounded corners to an image."""
...
def style_inner_eyes(img: Image) -> Image:
    """Create a mask for the inner eyes of the QR code."""
    img_size = img.size[0]
    eye_size = 70  # default
...
def style_outer_eyes(img: Image) -> Image:
    """Create a mask for the outer eyes of the QR code."""
    img_size = img.size[0]
    eye_size = 70  # default
    quiet_zone = 40  # default
    mask = Image.new("L", img.size, 0)
...
# qr_img.save("src/assets/qr-code.png")
# qr_img
